...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `Sqlx.swiftmodule/`
...
//      `import Sqlx` above would fail at compile time.
//
//   2. The static archive `libSqlx.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "Sqlx" }` and
//      `flattenPackage = "io.github.kotlinmania.sqlx"` configuration in
...
final class SqlxExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "Sqlx swift module imported cleanly")
    }
}
