# Claude Code Project Instructions — sqlx-kotlin

## Project Overview

This is **sqlx-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`sqlx`](https://crates.io/crates/sqlx).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/sqlx/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/sqlx>
